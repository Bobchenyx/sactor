#!/usr/bin/env python3
"""
批量翻译所有 C 文件 - 使用 SACToR 原生验证机制
包括 argv 和 scanf 目录中的所有文件
"""
...
class SactorAllTranslator:
    """使用 SACToR 原生验证的完整批量翻译器"""
    ...
    def __init__(self, config_path: str):
        ...
        # 创建临时工作目录
        self.temp_dir = tempfile.mkdtemp(prefix='sactor_all_')
    ...
    def create_sactor_test_config(self, c_file_path: str, output_dir: str) -> tuple[str, str]:
        """根据程序类型创建 SACToR 格式的测试配置"""
        program_name = os.path.basename(c_file_path)
        
        # 根据目录和文件名判断程序类型
        if "scanf" in c_file_path:
            # scanf 类型的程序 - 通常需要格式化输入
            test_samples = [
                {"input": "123", "output": ""},
                {"input": "456", "output": ""},
                {"input": "789", "output": ""},
    ...
    def batch_translate_all(self, dataset_dirs: List[str], output_base_dir: str) -> Dict:
        """批量翻译所有目录中的 C 文件"""
        all_c_files = []
        ...
        for dataset_dir in dataset_dirs:
            if os.path.exists(dataset_dir):
                ...
                print(f"📁 从 {dataset_dir} 找到 {len(c_files)} 个 C 文件")
        
        total_files = len(all_c_files)
        print(f"🚀 开始批量翻译总计 {total_files} 个 C 文件")
        ...
        for i, c_file_path in enumerate(all_c_files):
            ...
            if result['success']:
                ...
                print(f"❌ 失败: {os.path.basename(c_file_path)} - {result['error']}")
...
def main():
    ...
    sactor_config = "/home/changdi/sactor/sactor.toml"
    output_base_dir = "/home/changdi/sactor-datasets/sactor_all_translations"
    ...
    # 创建完整翻译器
    translator = SactorAllTranslator(sactor_config)
    
    try:
        # 批量翻译所有文件
        results = translator.batch_translate_all(dataset_dirs, output_base_dir)
        ...
        # 保存详细结果
        results_file = os.path.join(output_base_dir, "sactor_all_results.json")
