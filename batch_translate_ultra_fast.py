...
class UltraFastTranslator:
    ...
    def __init__(self):
        """初始化翻译器"""
        self.temp_dir = tempfile.mkdtemp(prefix='sactor_ultra_')
    ...
    def is_simple_program(self, c_file_path: str) -> bool:
        """判断是否为简单程序"""
        try:
            ...
            has_complex_logic = any(keyword in content for keyword in ['switch(', 'case ', 'struct ', 'typedef'])
            
            # 简单程序标准
            if line_count > 50 or has_functions or has_complex_logic:
                return False
    ...
    def translate_and_verify(self, c_file_path: str, output_dir: str) -> Dict:
        ...
        try:
            # 检查是否为简单程序
            if not self.is_simple_program(c_file_path):
                return {
                    'success': False,
                    'error': "跳过复杂程序",
                    'verification': None,
            ...
            print(f"⚡ 超快速翻译: {os.path.basename(c_file_path)}")
    ...
    def batch_translate(self, output_base_dir: str, max_files: int = None) -> Dict:
        ...
        print(f"🚀 开始超快速批量翻译 {total_files} 个 C 文件")
        ...
        for i, c_file_path in enumerate(all_c_files):
            ...
            results['failed'] = results['total'] - results['success'] - results['skipped']
            
            # 计算平均时间
            processed = results['success'] + results['failed']
            if processed > 0:
                results['avg_duration'] = results['total_duration'] / processed
            
            # 每处理 3 个文件保存一次进度（更频繁）
            if (i + 1) % 3 == 0:
                self._save_progress(results, output_base_dir, i + 1)
                print(f"📊 进度: {i+1}/{total_files}, 成功: {results['success']}, 跳过: {results['skipped']}, 平均时间: {results['avg_duration']:.1f}秒/个")
...
def main():
    """主函数"""
    # 配置路径
    output_base_dir = "/home/changdi/sactor-datasets/sactor_ultra_fast_translations"
    ...
    print(f"⚡ 超快速设置:")
    print(f"   - Docker 超时: 60秒 (极短超时)")
    print(f"   - 跳过复杂程序: 自动跳过 >50行 或多函数程序")
    print(f"   - 进度保存: 每3个文件")
    print(f"   - 预计平均时间: 15-30秒/个 (超快速)")
    ...
    try:
        ...
        # 保存详细结果
        results_file = os.path.join(output_base_dir, "sactor_ultra_fast_results.json")
