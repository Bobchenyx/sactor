#!/usr/bin/env python3
"""
翻译 CodeNet 每题第5到第20个 Accepted 的 C 文件
支持断点续传、进度记录、增量翻译
"""
...
class CodeNet5To20Translator:
    def __init__(self, workers=10, num_tests=6, resume=True):
        ...
        self.output_base_dir = "/home/changdi/sactor/translated_rust_5_to_20"
        
        # 进度文件
        self.progress_file = "/home/changdi/sactor/translation_progress_5_to_20.json"
        self.log_file = "/home/changdi/sactor/translation_log_5_to_20.txt"
        ...
        print("=" * 80)
        print("🚀 CodeNet 第5-20批 Accepted C → Rust 翻译工具")
        print("=" * 80)
        ...
        print(f"📊 翻译范围: 每题第5到第20个Accepted (索引4-19)")
        print("=" * 80)
    ...
    def collect_accepted_c_files_5_to_20(self):
        """收集第5到第20个 Accepted 的 C 文件"""
        print("\n🔍 扫描 CodeNet 第5-20批 Accepted C 文件...")
        ...
        for metadata_file in metadata_files:
            ...
            try:
                ...
                # 为第5到第20个 Accepted 的 C 文件创建任务
                # 索引4-19 (Python从0开始，所以第5个是索引4)
            ...
            if processed % 100 == 0:
                print(f"   进度: {processed}/{total_problems} ({processed*100//total_problems}%) "
                      ...
                      f"跳过前4个 {skipped_pre_existing} 个", flush=True)
        
        print("\n✅ 扫描完成:")
        print(f"   - 共收集 {len(tasks)} 个待翻译任务")
        print(f"   - 跳过 {self.skipped} 个已完成任务")
        print(f"   - 跳过 {skipped_pre_existing} 个前4个Accepted文件")
    ...
    def run(self):
        """执行批量翻译"""
        # 收集任务
        tasks = self.collect_accepted_c_files_5_to_20()
        if not tasks:
            print("❌ 没有找到任何待翻译任务")
            return
...
def main():
    parser = argparse.ArgumentParser(description='翻译CodeNet每题第5-20个Accepted的C文件到Rust')
    ...
    translator = CodeNet5To20Translator(workers=args.workers, num_tests=args.num_tests, resume=args.resume)
    translator.run()
