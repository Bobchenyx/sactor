...
class CorrectDataTranslator:
    ...
    def _get_model_info(self) -> str:
        ...
        try:
            import re
            # 读取sactor_1.toml配置文件
            config_path = "/home/changdi/sactor/sactor_1.toml"
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    elif 'llm = "Qwen"' in content:
                        # 查找[Qwen]部分下的model配置
                        qwen_section = re.search(r'\[Qwen\](.*?)(?=\[|$)', content, re.DOTALL)
                        if qwen_section:
                            model_match = re.search(r'model = "([^"]+)"', qwen_section.group(1))
                            if model_match:
                                return model_match.group(1)
            return "Unknown"
    ...
    def translate_with_sactor_docker(self, c_file_path: str, output_dir: str, test_task_path: str) -> Dict:
        ...
        try:
            # 直接使用绝对路径，挂载整个数据集目录
            sactor_config = "/home/changdi/sactor/sactor_1.toml"
            cmd = [
                "docker", "run", "--rm",
                "-v", f"{sactor_config}:/app/sactor_1.toml",
